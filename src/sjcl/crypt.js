...
sjcl.crypt.scrypt.blockMix = function(blocks) {
...
sjcl.crypt.scrypt.ROMix = function(block, N) {
  var X = block.slice(0),
      V = [],
      self = sjcl.crypt.scrypt;
...
sjcl.crypt.scrypt.reverse = function (words) { // Converts Big <-> Little Endian words
  for (var i in words) {
...
sjcl.crypt.scrypt.blockcopy = function (S, Si, D, Di, len) {
  var i;
...
sjcl.crypt.scrypt.blockxor = function(S, Si, D, Di, len) {
  var i;
